...
function demoData() {
  ...
  return {
    ...
    market: {
      symbol: "BTCUSD", bid: price, ask: price + .19, spreadPoints: 19,
    ...
    alerts: {
      ...
      journal: { enabled: true, status: "NOT WRITTEN", file: "activity-btcusd.csv", lastEvent: "", lastTime: 0 }
    },
    news: {
...
function render(data, live = false) {
  ...
  setText("#symbol", data.market?.symbol || "BTCUSD");
  setText("#heroInstrument", data.market?.symbol || "BTCUSD");
  setText("#chartInstrument", data.market?.symbol || "BTCUSD");
...
function openDatabase() {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open("ExnessBitcoinGuardDashboard", 1);
...
async function connectLive() {
  try {
    ...
    const [handle] = await window.showOpenFilePicker({
      multiple: false,
      types: [{ description: "Bitcoin Guard live feed", accept: { "application/json": [".json"] } }]
    });
